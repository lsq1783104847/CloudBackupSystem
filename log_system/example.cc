...
{
    LOG_SYSTEM_LOG_DEBUG(logger, "日志输出测试-DEBUG");
    LOG_SYSTEM_LOG_INFO(logger, "日志输出测试-INFO");
    LOG_SYSTEM_LOG_WARN(logger, "日志输出测试-WARN");
    LOG_SYSTEM_LOG_ERROR(logger, "日志输出测试-ERROR");
    LOG_SYSTEM_LOG_FATAL(logger, "日志输出测试-FATAL");
    for (int i = 0; i < 20000; i++)
        LOG_SYSTEM_LOG(logger, log_system::Level::ERROR, "日志输出测试-%d", i);
}
...
void test1()
{
    ...
    log_system::delete_logger("asynlogger3");
    ...
    if (asynlogger3 == nullptr)
    {
        LOG_SYSTEM_LOG_FATAL(asynlogger1, "asynlogger3 delete done");
        LOG_SYSTEM_LOG_FATAL(asynlogger2, "asynlogger3 delete done");
    }
...
void test3()
{
    ...
    LOG_SYSTEM_LOG_DEBUG(rootlogger, "日志输出测试-DEBUG");
    LOG_SYSTEM_LOG_INFO(rootlogger, "日志输出测试-INFO");
    LOG_SYSTEM_LOG_WARN(rootlogger, "日志输出测试-WARN");
    LOG_SYSTEM_LOG_ERROR(rootlogger, "日志输出测试-ERROR");
    LOG_SYSTEM_LOG_FATAL(rootlogger, "日志输出测试-FATAL");
    for (int i = 0; i < 5; i++)
        LOG_SYSTEM_LOG(rootlogger, log_system::Level::WARN, "日志输出测试-%d", i);
}
